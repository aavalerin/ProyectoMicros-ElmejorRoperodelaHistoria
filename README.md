# 👕 Sistema de Percheros Circulares Inteligentes

**Proyecto Final — MT7003 Microcontroladores y Microprocesadores**
Instituto Tecnológico de Costa Rica (ITCR)
Ingeniería Mecatrónica — I Semestre 2026

---

## 👥 Integrantes

* Sebastián Emilio Campos Ramírez
* María Jimena Araya Arguedas
* José Eduardo Beita Elizondo
* Javier Araya
* Aarón Valerín Páez

---


## Descripción del Proyecto

Este proyecto de diseño en ingeniería propone el desarrollo de un sistema de **percheros circulares inteligentes**.

El sistema está inspirado en percheros rotatorios donde el usuario **no necesita desplazarse** para acceder a las prendas. En su lugar, el sistema posiciona automáticamente el perchero solicitado frente al usuario.

sdkfjsdkfjsdlfjsldfkj le podemos agregar detalles luego

---
## 🏫 Información del Curso

Curso: MT-7003 Microprocesadores y Microcontroladores
Profesor: Ing. Rodolfo Piedra Camacho
Institución: Instituto Tecnológico de Costa Rica
Semestre: I Semestre 2026
